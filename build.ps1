...
#   .\build.ps1 3dsx      # make build/myfarm.3dsx only
#   .\build.ps1 clean
#
param(
    [string]$Target = "cia"
...
podman build -t myfarm-builder -f "$PSScriptRoot\Containerfile" $PSScriptRoot
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

podman run --rm -v "${PSScriptRoot}:/project" myfarm-builder make $Target
exit $LASTEXITCODE
